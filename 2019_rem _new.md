...
**Эйлерова характеристика поверхности.** Обозначим $\alpha_0, \alpha_1, \alpha_2$ – число вершин, ребер и треугольных граней в триангуляции $Z$ поверхности. Эйлерова характеристика Z равна $\chi(Z) = \alpha_0 - \alpha_1 + \alpha_2$ (7.1)
...
$Z_s = F_e(X_s, Y_s) = a - \sqrt{r^2 - X_s^2 - Y_s^2}$ (7.2)
...
$F_i(X_s, Y_s, Z_s) = X_s^2 + Y_s^2 + (Z_s - a)^2 - r^2 = 0$, где $a - r > f$ (7.3)
...
$K_G(P) = \lim_{\varepsilon \to 0}
\frac{A(R_\varepsilon)}{A(S_\varepsilon)}$ (7.8)
...
$k_{\eta}(p) = \lambda_1 \cdot \cos^2(\eta) + \lambda_2 \cdot \sin^2(\eta)$ (7.9)
...
$k_\eta = k_c \cdot \cos(n_p, n_c)$ (7.10)
...
$k_3 = \frac{\min(k_1, |k_2|)}{\max(k_1, |k_2|)}$ (7.11)
...
$x_u = \frac{f X_s}{Z_s}, \quad y_u = \frac{f Y_s}{Z_s}$ (7.14)
...
$\frac{d}{\sin \alpha} = \frac{b}{\sin \gamma}$ (7.15)

Отсюда следует, что

$$
d = \frac{b \cdot \sin \alpha}{\sin \gamma} = \frac{b \cdot \sin \alpha}{\sin (\pi - \alpha - \beta)} = \frac{b \cdot \sin \alpha}{\sin (\alpha + \beta)}
$$
...
$Z = \frac{X}{x} \cdot f = \tan \alpha \cdot (b - X) \quad \text{и} \quad X \cdot \left( \frac{f}{x} + \tan \alpha \right) = \tan \alpha \cdot b.$
(7.18)

В результате получаем решение

$$
X = \frac{\tan \alpha \cdot b \cdot x}{f + x \cdot \tan \alpha}, \quad
Y = \frac{\tan \alpha \cdot b \cdot y}{f + x \cdot \tan \alpha}, \quad
Z = \frac{\tan \alpha \cdot b \cdot f}{f + x \cdot \tan \alpha}.
$$
...
$$
Z_s = \frac{f \cdot X_s}{x_{uL}} = \frac{f \cdot (X_s - b)}{x_{uR}}.
$$
...
Матрица поворота на угол $-\theta$ вокруг оси $Y$:
...
Для левой камеры сначала выполняется параллельный перенос на вектор 
$\left[ -\frac{b}{2}, 0, 0 \right]^T$, т.е. на величину $b/2$ влево:

$\left[ X - \frac{b}{2}, Y, Z \right]^T$

(7.26)

а затем поворот на угол $-\theta$ вокруг оси $Y$. Полное аффинное преобразование имеет вид:
...
Аналогично для правой камеры: сначала перенос на $\left[ \frac{b}{2}, 0, 0 \right]^T$, затем поворот на угол $\theta$ вокруг оси $Y$:
...
$(-x_{uL}\sin(\theta) - f\cos(\theta))\,X + (x_{uL}\cos(\theta) - f\sin(\theta))\, Z = + -\left(\frac{b}{2}x_{uL}\sin(\theta) + \frac{b}{2}f\right)$ (7.33)

$(-y_u\sin(\theta))\,X - f\,Y + (y_u\cos(\theta))\,Z = \frac{b}{2}y_u\sin(\theta)$ (7.34)

$(x_{uR}\sin(\theta) - f\cos(\theta))\, X + (x_{uR}\cos(\theta) + f\sin(\theta))\,Z = + -\left(\frac{b}{2}x_{uR}\sin(\theta) - \frac{b}{2}f\right) $ (7.35)
...
$a_1 X + a_3 Z = a_0$ (7.36)

$b_1 X + b_2 Y + b_3 Z = b_0$ (7.37)

$c_1 X + c_3 Z = c_0$ (7.38)
...
$s^T \cdot n_p = \|s\|_2 \cdot \|n_p\|_2 \cdot \cos \alpha$ (7.39)

$\cos \alpha = \frac{s^T \cdot n_p}{\|s\|_2 \cdot \|n_p\|_2}$  (7.40)
...
$\eta(P) = \rho(P) \cdot \frac{E_L}{\pi}$ (7.41)
...
$R(P) = \eta(P) \cdot \frac{\mathbf{s}_L^T \cdot \mathbf{n}_P}{\|\mathbf{s}_L\|_2 \cdot \|\mathbf{n}_P\|_2} = \eta(P) \cdot \frac{a_L a + b_L b + 1}{\sqrt{a_L^2 + b_L^2 + 1} \cdot \sqrt{a^2 + b^2 + 1^2}}.$ (7.42)
...
$u_i = R_i(P) = \frac{\eta(P)}{c} \cdot \frac{\mathbf{s}_i^T \cdot     \mathbf{n}_P}{\|\mathbf{s}_i\|_2 \cdot      \|\mathbf{n}_P\|_2}.$ (7.43)
...
$u_i = E_i \frac{\rho(P)}{c\pi} \cdot \frac{s_i \cdot n_P}{|s_i| \cdot |n_P|}$ (7.44)
...
$\rho(P) \cdot n_P \cdot (E_1 u_2 |s_2| \cdot s_1 - E_2 u_1 |s_1| \cdot s_2) = 0$ (7.45)
...
$n_P \cdot (E_1 u_3 |s_3| \cdot s_1 - E_3 u_1 |s_1| \cdot s_3) = 0$ (7.46)

В итоге получается, что вектор $\mathbf{n}_P$ коллинеарен векторному произведению

$n_P \parallel (E_1 u_2 |s_2| \cdot s_1 - E_2 u_1 |s_1| \cdot s_2) \times (E_1 u_3 |s_3| \cdot s_1 - E_3 u_1 |s_1| \cdot s_3)$ (7.47)
...
\textbf{Направление на источники и обратный ФАС.} Для калибровки направлений на три источника света $\mathbf{s}_i$ применяется обратный фотометрический метод анализа стереоизображений. Используется калибровочная сфера, обладающая (приблизительно) ламбертовской отражательной способностью и равномерным альбедо (рис. 7.24). Сфера располагается примерно в том же месте, где впоследствии нужно будет реконструировать нормали к поверхности объекта. Точность калибровки будет тем выше, чем больше сфера, т.е. желательно, чтобы она заполняла всё изображение, снятое неподвижной камерой.

Мы выделяем в изображении круговую границу сферы. Это позволит вычислить нормали к поверхности сферы во всех точках $P$, проецируемых в позиции пикселей внутри этой окружности. В силу ожидаемой неточности по отношению к ламбертовской отражательной способности (см. приближённые изолинии интенсивности на рис. 7.24 справа) понадобится больше трёх точек $P$ (скажем, примерно 100 равномерно распределённых точек с нормалями внутри круговой области, занятой сферой в изображении, которое было снято при освещении источником света $i$), чтобы определить направление $\mathbf{s}_i$ методом наименьших квадратов с использованием уравнения (7.44): нам известны значения $u_i$ и нормали $\mathbf{n}_P$, а требуется найти направление $\mathbf{s}_i$. Таким же образом мы измерим отношение интенсивностей $E_i$ всех трёх источников света.
...
**Реконструкция альбедо.** Рассмотрим уравнение (7.44) для $\( i = 1, 2, 3 \)$. Известны все три значения $\( u_i \)$ в позиции пикселя $\( p \)$, являющейся проекцией пространственной точки $\( P \)$. Также известны приближённые значения единичных векторов $\(\mathbf{s}_i^o\) и \(\mathbf{n}_P^o\)$. Мы измерили относительные интенсивности трёх источников света.


Осталось одно неизвестное — $\(\rho(P)\)$.
 В описанном выше методе ФАС, не зависящем от альбедо, мы комбинировали первое и второе изображения, а затем первое и третье. Можно еще скомбинировать второе изображение с третьим. Благодаря этим действиям мы можем получить надежную оценку $\(\rho(P)\)$
...
**Методы локального интегрирования.** В непрерывном случае значения функции глубины $\( Z = Z(x, y) \)$, определённой на односвязном множестве, можно реконструировать, начав с одной точки $\((x_0, y_0)\)$ с начальным значением $\( Z(x_0, y_0) \)$ и проинтегрировав градиенты вдоль пути $\(\gamma\)$, целиком проходящего внутри этого множества. Оказывается, что интегрирование по различным путям даёт одинаковое значение в точке $\((x_1, y_1)\)$. Если быть точным, глубина $\( Z \)$ должна ещё удовлетворять условию интегрируемости.


$\frac{\partial^2 Z}{\partial x \partial y} = \frac{\partial^2 Z}{\partial y \partial x}\$ (7.48)
...
$\frac{\partial Z}{\partial x}(x, y) = a_{x, y}\$ (7.49)

$\frac{\partial Z}{\partial y}(x, y) = b_{x, y}\$ (7.50)
...
$Z(x + 1, y + 1) = Z(x, y + 1) + \frac{1}{2}\left(a_{x, y + 1} + a_{x + 1, y + 1}\right)$ (7.51)


Аналогично получаем рекурсивное соотношение для пикселей $(x + 1, y)$ и $(x + 1, y + 1)$ :

$Z(x + 1, y + 1) = Z(x + 1, y) + \frac{1}{2}\left(b_{x + 1, y} + b_{x + 1, y + 1}\right).$ (7.52)


Сложив оба равенства и поделив результат на 2, получим

$Z(x + 1, y + 1) = \frac{1}{2}\bigl(Z(x, y + 1) + Z(x + 1, y)\bigr) + \frac{1}{4}\bigl(a_{x, y + 1} + a_{x + 1, y + 1} + b_{x + 1, y} + b_{x + 1, y + 1}\bigr).$ (7.53)
...
$Z(x, 1) = Z(x - 1, 1) + a_{x - 1, 1},$ (7.54)


$Z(1, y) = Z(1, y - 1) + b_{1, y - 1},$ (7.55)
...
$Z(x - 1, N_{\text{rows}}) = Z(x, N_{\text{rows}}) - a_{x, N_{\text{rows}}},$ (7.56)


$Z(N_{\text{cols}}, y - 1) = Z(N_{\text{cols}}, y) - b_{N_{\text{cols}}, y}.$ (7.57)


Затем выполняется горизонтальный проход по изображению с применением рекуррентного соотношения:

$Z(x-1, y-1) = \frac{1}{2} \bigl(Z(x-1, y) + Z(x, y-1)\bigr) - \frac{1}{4} \bigl(a_{x-1, y} + a_{x, y} + b_{x, y-1} + b_{x, y}\bigr).$ (7.58)
...
$E_{\text{data}}(Z) = \sum_{\Omega} \bigl[(Z_x - a)^2 + (Z_y - b)^2\bigr] + \lambda_0 \sum_{\Omega} \bigl[(Z_{xx} - a_x)^2 + (Z_{yy} - b_y)^2\bigr].$ (7.60)


с членом гладкости

$E_{\text{smooth}}(Z) = \lambda_1 \sum_{\Omega} \left(Z_x^2 + Z_y^2\right) + \lambda_2 \sum_{\Omega} \left(Z_{xx}^2 + 2Z_{xy}^2 + Z_{yy}^2\right),$ (7.61)
\]
...
$E_{\text{total}}(Z) = E_{\text{data}}(Z) + E_{\text{smooth}}(Z)$ (7.62)
...
$\mathcal{Z}(u, v) = \frac{1}{|\Omega|} \sum_{\Omega} Z(x, y) \cdot \exp \left[ -i 2 \pi \left( \frac{x u}{N_{\text{cols}}} + \frac{y v}{N_{\text{rows}}} \right) \right],$ (7.63)


а обратное преобразование Фурье:

$Z(x, y) = \sum_{\Omega} \mathcal{Z}(u, v) \cdot \exp \left[ i 2 \pi \left( \frac{x u}{N_{\text{cols}}} + \frac{y v}{N_{\text{rows}}} \right) \right],$ (7.64)
...
$(u^2 \mathcal{Z} + i u \mathcal{A} + v^2 \mathcal{Z} + i v \mathcal{B}) + \lambda_0 (u^4 \mathcal{Z} + i u^3 \mathcal{A} + v^4 \mathcal{Z} + i v^3 \mathcal{B}) + \lambda_1 (u^2 + v^2) \mathcal{Z} + \lambda_2 (u^4 + 2 u^2 v^2 + v^4) \mathcal{Z} = 0.$ (7.67)


После перегруппировки членов получаем:

$\bigl[\lambda_0 (u^4 + v^4) + (1 + \lambda_1)(u^2 + v^2) + \lambda_2 (u^2 + v^2)^2\bigr] \mathcal{Z}(u, v) + i(u + \lambda_0 u^3) \mathcal{A}(u, v) + i(v + \lambda_0 v^3) \mathcal{B}(u, v) = 0.$ (7.68)


Решаем это уравнение при условии $(u, v) \neq (0, 0)$:

$\mathcal{Z}(u, v) = \frac{-i(u + \lambda_0 u^3) \mathcal{A}(u, v) - i(v + \lambda_0 v^3) \mathcal{B}(u, v)}{\lambda_0 (u^4 + v^4) + (1 + \lambda_1)(u^2 + v^2) + \lambda_2 (u^2 + v^2)^2}.$ (7.69)
